...
From the above graph, we can see that node 1, 12, 13 and 19 stand out as they have higher degree centrality than the rest. Below code reviews these important nodes.

```{r}
vertex_attr(mc2_graph_306170_agg, index = V(mc2_graph_306170_agg)$size > quantile_graph_306170_agg[5])
```

`centrality_betweeness()` measure the centrality of every nodes by calculating how many shortest path containing that node in the network. This can be visualized to give you information about how important each node in the term of graph/network connectivity. For this case, we can see how important a node is between trades of other companies.
...
g <- mc2_graph_306170 %>%
  ...
  geom_node_point(aes(colour = id,size=betweenness_centrality)) 
...
Check the temporal trading frequencies of the companies with high degree of centrality.

```{r}
p1 <- ggplot(data = MC2_edges_aggregated_306170, aes(x = Month_Yr, y = weights, group = 19)) +
  geom_line()+
...
p2 <- ggplot(data = MC2_edges_aggregated_306170, aes(x = Month_Yr, y = weights, group = 13)) +
  geom_line()+
  ggtitle("Temporal Evolution of Weights")  +
  theme(axis.title.y= element_text(angle=90), axis.ticks.x= element_blank(),
        axis.line= element_line(color= 'grey'),
